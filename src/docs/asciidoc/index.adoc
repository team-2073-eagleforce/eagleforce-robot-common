...
It is *strongly* recommended to use the `AbstractRobotDelegator`.
This will automatically run the `PropertyLoader` (reload the properties) on an async periodic loop every 5 seconds.

The `PropertyLoader` registers itself with the `PeriodicRunner` automatically and the `AbstractRobotDelegator`
...
[TIP]
====
If you use the `AbstractRobotDelegator` a restart is not even required as properties are automatically reloaded every 5 seconds.
====
