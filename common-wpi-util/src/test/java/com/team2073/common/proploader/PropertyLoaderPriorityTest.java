...
public class PropertyLoaderPriorityTest extends BaseWpiTest {
    ...
    // -In each "scenario" directory, there should be ONLY ONE file with the correct property value ("foo-expected-value")
    // -The other files have invalid values and if the PropertyLoader loads values from these incorrect files, the tests will fail

    // -Within a scenario directory there are two directories
    //      -A-source-code-files
    //          -These are files that users of the library have in their source code (src/main/resources)
    //          -These files do not get copied anywhere, they are accessed directly from this directory during tests
    //      -B-external-files
    //          -These are meant to resemble files that are external to the source code (Ex: in the user directory)
    ...
    private void runTest() throws IOException {
        ...
        loader.setSourceCodeConfDirPath(sourceCodeConfDirPath);
