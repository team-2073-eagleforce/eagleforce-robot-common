...
public class Trajectory {
    ...
    public static class Pair {
        public Pair(Trajectory left, Trajectory right, Trajectory center) {
            this.left = left;
            this.right = right;
            this.center = center;
        }
    ...
    public Trajectory cutPath(int segments, boolean front, boolean back) {
        int multiplier = 0;
        if (front) {
            multiplier++;
        }
    ...
    public void relativePosition(double startingPos) {
        for (int i = 0; i < getNumSegments(); ++i) {
            segments_[i].pos += startingPos;
        }
    }
